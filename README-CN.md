...
**Qwen-VL-Narrator** 是一个影视领域专家模型，旨在为影视作品的短视频片段生成人物、场景和拍摄手法层面的细粒度描述。能够应用于视频检索、摘要、理解、细粒度标注等场景。
...
* **小尺寸参数量**：该模型基于 Qwen2-VL 7B 微调得来，能够方便地部署在单张 H20、L20 乃至 5090 上。
...
本模型为 Qwen2-VL 微调模型，使用方式与原版模型无异，在调用时，提供视频内容并使用下列提示词即可。
...
* 由于 Qwen2-VL 架构的局限性，模型无法对音频进行描述。
